...
"""
UIFuncitons简介:
    1. toggleMenu(self, enable)：左侧菜单栏展开
    2. settingBox(self, enable)：右侧设置栏展开
    3. maximize_restore(self)：最大化/还原
    4. uiDefinitions(self)：窗口控制
    5. resize_grips(self)：控制窗口四边拉伸
    6. shadow_style(self, widget, Color)：添加阴影
"""
...
class UIFuncitons(MainWindow):
    ...
    def shadow_style(self, widget, Color):
        shadow = QGraphicsDropShadowEffect(self) # 实例化阴影类
        shadow.setOffset(8, 8)  # 这是阴影的偏移量
        shadow.setBlurRadius(38)    # 阴影的模糊半径
        shadow.setColor(Color)    # 阴影的颜色
